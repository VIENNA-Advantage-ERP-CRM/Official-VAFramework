...
(function (VIS, $) {
    ...
    VIS.VISPendingChecklist = function () {
        ...
        this.Initalize = function () {
            ...
            createBusyIndicator();
            checklistRecWidget = $('<div><h4></h4></div>' +
                '<div class="VIS_checklist_records" id ="VIS_checklistRecords_' + widgetID + '">' +
                '<div class="vis-record-col" id ="VIS_pendingsRecords_' + widgetID + '">' +
            ...
            pendingRecords = widgetContainer.find('.vis-record-col');
        ...
        function createWindowUsers() {
            $.ajax({
                url: VIS.Application.contextUrl + "PendingChecklist/PendingCheckList",
                data: {
                    pageSize: pageSize,
                    pageNo: pageNo,
                    getAll: getAll
        ...
        function updateUI() {
            ...
            // Update header
            widgetContainer.find('h4').text(`${VIS.Msg.getMsg('VIS_PendingChecklist')}: ${allRecords[0].totalRecordCount}`);

            // Display records
            currentRecords.forEach((record, i) => {
                var colorClass = 'vis-color-' + ((i % 4) + 1);
                var pendingChecklistItems = `
                ...
                pendingRecords.append(pendingChecklistItems);
                //$bsyDiv[0].style.visibility = "hidden";
                showBusy(false);
            });
            ...
            // Add pagination UI
            //var paginationHtml = `
            //<div class="vis-tiles-pagination" id="pagination_${widgetID}">
            //    <i class="fa fa-arrow-circle-up vis_prevpage" id="prevPage_${widgetID}" aria-hidden="true"></i>
            //    <span class="vis-total-count" style="color: white;">${currentPage} / ${totalPages}</span>
            //    <i class="fa fa-arrow-circle-down vis_NxtPage" id="nextPage_${widgetID}" aria-hidden="true"></i>
            //    <i class="fa fa-list vis-show-more" id="showMore_${widgetID}" aria-hidden="true"></i>
            //</div>`;
            ...
            widgetContainer.find('.vis-show-more').on('click', function () {
                var popupContent = `                 
                       ...
                       <h3 class="VIS_popuptitle">${VIS.Msg.getMsg('VIS_PendingChecklist')}:${allRecords[0].totalRecordCount}</h3>
            ...
            pendingRecords.find('.vis-record-box').off('click')
            pendingRecords.find('.vis-record-box').on('click', function () {
                WindowName = $(this).attr('visWindowname');
                WindowId = $(this).attr('visWindowId');
                TableName = $(this).attr('visTableName');
                Record_ID = $(this).attr('visRecordId');

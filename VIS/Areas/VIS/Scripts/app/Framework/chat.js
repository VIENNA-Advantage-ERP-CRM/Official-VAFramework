        ...
        function init($container, windowNo, prop) {
            ...
            VIS.dataContext.getChatRecords(prop, function (result) {
                ...
                for (var chat in data.subChat) {
                    ...
                    // Delete edit icon on the top-right corner  vis-pencil ----data.subChat[0].ChatEntry_ID
                    // if (VIS.Env.getCtx().ctx["#IsAdmin"] == 'Y') {
                    /* str += '<span class="vis vis-pencil edit_chat" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Edit Chat" style="position:absolute; top:0; right: 5%;  cursor: pointer;" ></span>';
                      str += '<span class="vis vis-delete del_chat" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Delete Chat" style="position:absolute; top:0; right: 0;  cursor: pointer;" ></span>';*/
                    if (VIS.Application.isRTL) {
                        if (data.isDelete == 'Y') {
                            str += '<span class="vis vis-delete del_chat vis-chat-delete-icon" style="left:6%;" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Delete Chat"></span>';
                        }
                        str += '<span class="vis vis-pencil edit_chat vis-chat-edit-icon" style="left:1%;" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Edit Chat"></span>';

                    }
                    else {
                        if (data.isDelete == 'Y') {
                            str += '<span class="vis vis-delete del_chat vis-chat-delete-icon" style="right:6%;" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Delete Chat"></span>';
                        }
                        str += '<span class="vis vis-pencil edit_chat vis-chat-edit-icon" style="right:1%;" data-chatid="' + data.subChat[chat].ChatEntry_ID + '" title="Edit Chat"></span>';
                    }
                    ...
                    if (VIS.Application.isRTL) {
                        ...
                        //   str += '<span style="font-size: .75rem;padding-right:5px;white-space: pre-line;">' + VIS.Utility.encodeText(data.subChat[chat].ChatData) + '</span>';

                    }
                    else {

                        // str += '<span class="vis-chat-msg" style="font-size: .75rem;padding-right:5px;white-space: pre-line;">' + VIS.Utility.encodeText(data.subChat[chat].ChatData) + '</span>';
                        str += '<span class="vis-chat-msg" style="font-size: .75rem;padding-right:5px;white-space: pre-line;">' + VIS.Utility.encodeText(data.subChat[chat].ChatData);
                    }
            ...
            $container.on("click", ".edit_chat", function () {
                var $chatBox = $(this).closest(".vis-chatboxwrap");
                var chatId = $(this).data("chatid");
                var $chatTextSpan = $chatBox.find(".vis-chat-msg");
                var $editIcon = $chatBox.find(".edit_chat");
                var $delIcon = $chatBox.find(".del_chat");
                var $sendIcon = null;
                var $undoIcon = null;
 ...
 */
            ...
            $container.off("click", ".del_chat").on("click", ".del_chat", function (e) {
        ...
        function saveMsg(e) {
            ...
            self.prop.ChatText = text;            
            VIS.dataContext.saveChat(self.prop, busyCallback);
            // VIS.dataContext.saveChat(self.prop);
            // Assuming saveChat has a callback
        };

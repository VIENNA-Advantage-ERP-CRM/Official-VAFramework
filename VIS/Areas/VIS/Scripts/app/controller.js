 ...
 */
    ...
    GridTable.prototype.open = function (maxRows) {
        ...
        var dataIn = {
            ...
            treeID: this.treeID, treeNode_ID: this.treeNode_ID, card_ID: this.card_ID, ad_Tab_ID: this.AD_Tab_ID, tableName: this.gTable._tableName
        };
    ...
    GridTable.prototype.dataSaveL = function (manualCmd) {
        ...
        if (this.onlyCurrentDays == 0 && (this.IsMaintainVersions || this.maintainVersionFieldChanged(RowData, OldRowData))) {
            ...
            if (OldRowData["updatedby"] == null) {
                ...
                var res = VIS.dataContext.getJSONData(VIS.Application.contextUrl + "JsonData/CheckVersions", { RowData: gridTableIn });
                if (res.result) {
    ...
    GridTable.prototype.fireDataStatusEEvent = function (AD_Message, info, isError, isWarn) {
        ...
        if (arguments.length === 1) {
            this.fireDataStatusEEvent(myarguments [0].value, arguments[0].name, true, false);
        }
        else {
            var e = this.createDSE();
            if (!isWarn)
